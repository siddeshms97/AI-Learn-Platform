            ...
            <div class='course-card'>
                <div class='course-card-body'>
                    ...
                    <div style='display: flex; gap: 1rem; margin-top: 1rem;'>
                        ...
                        <span class='badge badge-primary'>💪 {len([l for l in course_detail['lessons'] if l.get('exercises')])} exercises</span>
                    </div>
                </div>
            </div>
            """, unsafe_allow_html=True)
...
def show_course_detail(user, course_key):
    ...
    for idx, lesson in enumerate(lessons, 1):
        ...
        with col2:
            if st.button("Read", key=f"lesson_{course_key}_{idx}", use_container_width=True):
                show_lesson(user, course_key, idx - 1)
...
def show_lesson(user, course_key, lesson_idx):
    ...
    if lesson.get('exercises'):
        ...
        for exc_idx, exercise in enumerate(lesson['exercises'], 1):
            with st.expander(f"Exercise {exc_idx}: {exercise['title']} ({exercise['difficulty']})"):
