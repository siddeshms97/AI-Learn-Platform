...
def show_quizzes(user):
    ...
    if "selected_quiz" in st.session_state:
        show_quiz_detail(st.session_state.selected_quiz)
        return
    ...
    for quiz in quizzes:
        ...
        with col2:
            if st.button(
                "📝 Start", key=f"quiz_{quiz['title']}", use_container_width=True
            ):
...
def show_quiz_detail(quiz):
    ...
    st.info(
        f"📋 This quiz has {quiz['questions']} questions. You need {quiz['passing_score']}% to pass."
    )
    ...
    for idx, q in enumerate(questions[:3]):
        st.markdown(
            f"<h4>Question {idx + 1}: {q['question']}</h4>", unsafe_allow_html=True
        )
    ...
    with col1:
        if st.button("⬅️ Exit Quiz", use_container_width=True):
            del st.session_state.selected_quiz
            st.rerun()
